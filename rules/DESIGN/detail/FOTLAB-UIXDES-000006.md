...
- The state belongs to the feature's lower layer, not to the composition — the same place the library already keeps `ListSelectionOfLibrary` (`FOTLAB-UIXDES-000004` R3). The screen reads it; it never mirrors or re-derives it.
...
- Back precedence is: **drawer → selection mode → the screen's own back step** (for the library: leaving the current directory). One back press never does two of these.
...
- In selection mode a screen may show a **batch action bar** holding the operations that apply to the selection — the library's 导出 / 删除, and for a file screen copy, move, rename, share and the like.
...
- `app/src/main/kotlin/io/github/fotlab/fotlab/feature/library/LibraryScreen.kt` — the top bar states of R3 and the back precedence of R4
- `app/src/main/kotlin/io/github/fotlab/fotlab/feature/library/LibraryCore.kt` — owns the mode and the selection set (R1)
...
- Q1 — Does the library keep deriving selection mode from "the selection is not empty", the way it does today, or does it adopt the explicit mode flag of R1? **TBD.**
