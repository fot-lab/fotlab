...
- Inside the module region the **top bar and the content region are siblings** — one above the other, neither overlapping the other. A module may place them either by hand (for example in a `Column`) **or by nesting its own Material3 `Scaffold` inside the shell's root `Scaffold`**, hosting the bar in a slot (`topBar` or `bottomBar`). A nested module `Scaffold` is permitted only when all three conditions hold:
