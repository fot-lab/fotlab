...
- 2026-09-14 — The first native binding is realised (design: `FOTLAB-STUDIO-000001`). Because the project remains a single Gradle module (`:app`), the "dedicated first-party native-integration module" R1 refers to is expressed as **source directories inside `app/`**, not a Gradle module: the Rust crate at `app/src/rust/binding/dnglab/rawler_fotlab` and the Kotlin facade at `app/src/kotlin/io/github/fotlab/fotlab/binding/dnglab/rawler_fotlab`. R1/R4 are enforced by construction: the crate is a standalone Cargo workspace that reaches upstream via a *path* dependency (`external/dnglab/rawler`), so `external/` holds no first-party code and upstream `Cargo.toml` is not modified. Only our binding carries a `_fotlab` name; anything built from upstream source keeps its upstream name.
