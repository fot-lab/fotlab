...
| `ACTION-PERFOR-000001` | 渲染管线性能审计总览 — 瓶颈归因、加速手段分级与五个待拍板项（8 个子条目的索引） | `PERFOR` | Observation | P1 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000001.md) |
| `ACTION-PERFOR-000002` | 渲染瓶颈归因 — FFI 调用开销被误判；真正的成本是像素搬运、重复拷贝与 PNG 编解码 | `PERFOR` | Observation | P1 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000002.md) |
| `ACTION-PERFOR-000003` | 预览始终走全分辨率 — native 侧无降采样路径（rawler superpixel 1/4 未接线），Coil 三处调用均未指定解码尺寸 | `PERFOR` | Observation | P1 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000003.md) |
| `ACTION-PERFOR-000004` | PNG 作为跨 FFI 的像素载荷 — 预览路径背负一次完整 deflate 与一次完整 inflate | `PERFOR` | Observation | P1 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000004.md) |
| `ACTION-PERFOR-000005` | 每次调参都重跑整条 develop 链 — 缓存粒度停在"已解码"，未到"已显影"（`FotDev`） | `PERFOR` | Observation | P1 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000005.md) |
| `ACTION-PERFOR-000006` | Library 里 RAW 没有真实缩略图 — Coil 解不了 RAW 落到 MIME 图标，rawler 的内嵌预览接口未被接线 | `PERFOR` | Observation | P2 | [detail](rules/REVIEW/detail/ACTION-PERFOR-000006.md) |
