...
| `FOTLAB-RAWLER-000011` | rawtrp_correct — 忠实移植 RawTherapee CA_correct_RT pass 2（手动 + 自动-from-fit），pre-demosaic 槽位；pass 1 拟合延迟至 C2 | `RAWLER` | Implemented | P2 | [detail](rules/REVIEW/detail/FOTLAB-RAWLER-000011.md) |
| `FOTLAB-RAWLER-000012` | Dehaze pipeline audit — shipped no-op at default (ceiling bound to percentile 0.01), hue-preserving multiplicative mask (implemented 4ccdb36), and dehaze-before-exposure underexposure failure (−4 EV → 6%) | `RAWLER` | Observation | P1 | [detail](rules/REVIEW/detail/FOTLAB-RAWLER-000012.md) |
| `FOTLAB-RAWLER-000013` | 高光溢出色偏端到端归因 — 归一化只做下限钳位（0..1 为未强制假设）、`whitepoint` 标称值致溢出至 1.205、exposure clip 因 identity 短路成为 UI 值属性、溢出是统一缩放故反使 PNG 支路变白、ProPhoto 越界至 [1.769, 1.081, 1.345] 而 log 支路不裁剪故保留色偏 | `RAWLER` | Observation | P1 | [detail](rules/REVIEW/detail/FOTLAB-RAWLER-000013.md) |
<!-- Next sequence per category: PREPIN 000002, LIBRND 000002, RAWLER 000009, KOTLIN 000008, FOTLAB-RAWLER 000014, ROLLBK 000002, PERFRM 000011. Append one row per new item; never reuse or renumber IDs. -->
