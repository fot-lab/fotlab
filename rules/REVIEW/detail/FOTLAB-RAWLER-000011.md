...
3. **Wire into `rawler_fotlab` pre-demosaic.** Add the path dep, call `correct_ca_bayer` on the `0..1` mosaic before `demosaic` (adjacent to `000009`'s stages), and surface `CaParams` through `DevelopParams` + the Kotlin hub (`000006`). Default = identity so existing renders are untouched.
