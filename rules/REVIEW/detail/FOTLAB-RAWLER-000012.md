...
- 2026-09-26 — Follow-up: per user decision, `develop_image` (`app/src/binding/rust/rawler_fotlab/src/develop.rs`) now applies **Exposure FIRST** among the pre-demosaic mosaic stages — order is `Exposure → Denoise → Dehaze → CA` (was `Denoise → Dehaze → CA → Exposure`). This realises recommendation #3's ordering decision and directly eliminates the F7 underexposure dehaze failure (dehaze now sees the exposure-corrected mosaic; the [0,1] histogram-bin clamp and `cap_tail` keep the estimate bounded for a positive EV). The Studio DevelopFilm bar (`StudioScreen.kt`, `StudioOperationBarDevelopFilm`) was reordered to match — the Exposure button moved to the first slot, before LCA. Local `cargo check` cannot finish because `rawler_fotlab`'s `rawalchemy` C++ bridge needs `cmake` (cloud-CI-only build); the change is a pure statement reorder (verified `apply_exposure` called once at the front, no signature change).
