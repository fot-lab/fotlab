...
- 2026-09-16 — Recommendations 1 and 2 applied. `LibraryCore.uriExists` now tests `it.count > 0`, with a KDoc warning above it explaining why `>= 0` is wrong (a provider answers with an *empty* cursor rather than `null`), so the defect cannot return by accident; the unused `val now` in `refresh()` was removed. No call-site change — `uriExists` has exactly one caller, and `refresh` is reached only from the two sync-icon callbacks in `LibraryScreen` / `LibraryScreenRecycle`. Verified by cloud CI only; no local Gradle build is permitted (`rules/ACTION.md`). Still `Observation` rather than `Implemented` because recommendation 3 below (a regression test proving a MediaStore-deleted file is recycled) is not written yet; promote once that test lands, or earlier if the owner accepts manual verification.
