...
- 2026-09-19 — **Kotlin consumption half wired end-to-end (Studio grade bar: Boost / LOG / LUT).** Until now no Kotlin caller of `develop_and_grade` existed. Added: (a) `bound::graded_to_png` — a third PNG encoder that quantizes the graded float buffer by direct clamp+×255 with **no transfer function** (the grade's log OETF already encoded it; routing it through `rawlerimagedeveloped_to_png`'s sRGB gamma would double-encode — this is the locked decision 4, "bit-shift to PNG"); (b) `RawlerImageLoaded::develop_and_grade_to_png` / `..._at_kelvin` (catch_unwind, same Kelvin WB override as the develop PNG variants, so a grade re-render carries the retained demosaic/exposure/WB); (c) `rawalchemy_fotlab::log_spaces()` over cxx + UniFFI `supported_log_spaces()` — enumerates upstream's `LOG_SPACES` keys (14 curves) instead of hard-coding a mirror; signature kept in sync across bridge/header/shim, `rust::Vec<rust::String>`. Kotlin: three `RawlerFotlabBridge` facades (grade PNG, grade PNG at Kelvin, log list; all `runCatching`-null-safe); `StudioEngine` grade state (`GradeSelection`, reset per file) with two independent forks — a demosaic/exposure/WB change renders the sRGB develop PNG as before, a Boost/LOG/LUT change re-develops the resident decode and renders the graded PNG; all-"none" returns to the develop fork (no dark linear ProPhoto frame); a grader failure (bad LUT) shows an error dialog and falls back to the develop fork. LUT is picked via unrestricted `OpenDocument()` (`*/*`) and copied from the `content://` URI to `cacheDir/grading-luts/<sha-256><ext>` because `loadCubeLUT` only reads real paths. UI: `StudioGradeBar` under the canvas (RAW-only) with three dropdown chips — Boost is two-state where the UI "none" means **boost explicitly OFF** (`enableBoost=false`), not upstream's engine default (which is on). `RawRoutingTest` untouched (grade defaults to inactive).
