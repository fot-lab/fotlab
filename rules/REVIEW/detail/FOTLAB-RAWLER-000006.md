...
pub struct GradeOverrides {
    ...
    pub metering_mode: Option<String>,  // None = no auto metering (base gain 1.0)
    pub ev_offset: f32,                 // 2^ev_offset on top of the base gain
...
rust::Vec<float> grade(rust::Slice<const float> data,
                       ...
                       float ev_offset, float target_gray,
  ...
  if (!mm.empty()) {
    if (!isMeteringModeSupported(mm)) throw std::runtime_error("grade: unsupported metering mode '" + mm + "'");
...
RawAlchemyCpp's `runGradingOnly` does `gain = computeAutoGain(img, mode) * 2^evOffset` — i.e. upstream meters **by default** (its `mode` falls back to `"matrix"`). Therefore the bridge exposes `metering_mode` as an optional override: `None` (the default) means *no* metering, leaving the base gain at 1.0 so `p.gain = 2^ev_offset` and rawler's already-applied as-shot exposure stays authoritative (no double exposure); `Some("matrix")` etc. reproduces upstream's own behaviour, with `target_gray` (upstream default 0.18) also overridable. The mode is validated with `isMeteringModeSupported`.
