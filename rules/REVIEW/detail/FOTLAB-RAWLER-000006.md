...
### cxx bridge constraints (each one cost a red CI run)
The bridge is the only place in the repo that uses cxx, and none of it had ever been compiled, so these came out one per run:
...
- 2026-09-19 — **First real CI compiles; build errors peeled one layer at a time.** cmake now *succeeds* for `aarch64-linux-android` (`librawalchemy_grading.a` is really produced with the NDK toolchain), which validates `RAWALCHEMY_SRC` discovery, the NDK toolchain passthrough and the grading-only subset. Three further defects surfaced in order: the build script did not itself compile (`cmake::Config::define` returns `&mut Self`, so chaining it off `Config::new()` made the builder a temporary → E0716); then the bridge was rejected by cxx (`unsafe extern "C++"`, `Result<T>` with no second parameter, and a hand-written `cpp/rawalchemy_api.h` because a pure `extern "C++"` bridge has no generated header). All recorded above under "cxx bridge constraints".
- 2026-09-19 — **The .so finally loads on-device and the grade bar gets full smoke coverage.** Two packaging defects the emulator smoke exposed (it is the first job that actually loads the .so): (1) the cdylib's `DT_NEEDED` on `libc++_shared.so` is NOT auto-packaged by AGP for hand-produced jniLibs — the native job now copies the NDK per-ABI `libc++_shared.so` into the jniLibs tree (`build.rs` stays on dynamic `c++`; static `c++_static` linked on arm64/x86/x86_64 but failed armv7 with `undefined version LIBC_N`, rust `compiler_builtins` version refs vs the `-lc++_shared` cxx's link-cplusplus injects). (2) Tests: engine-level Boost/LOG journey, real-UI chip journey, and the full **LUT journey** — `panasonicVLogAndDownloadedLutCubeBothTakeEffect` imports the Panasonic DC-S1R RW2, applies V-Log, then a real 33-point cube (`FLog2C_to_CLASSIC-Neg_VLog.cube` from the new `external/V-Log-Alchemy` submodule — a `*_VLog` creative cube expecting V-Log/V-Gamut input, matching our grade stage), fed through a MediaStore `content://` URI exactly like the SAF callback; asserts pixels move at each stage, the cube copy is SHA-256 identical in the cache, the LUT output carries chroma, and clearing restores frames deterministically. The workflow downloads the cube with a pinned SHA-256 and stages it into app-internal files via `installDebug` + `run-as cat > files/lut-fixture/...` (shell-created SD dirs are app-invisible; app reads of `/data/local/tmp` are SELinux-denied on API 36).
- 2026-09-19 — **LUT journey corrected: user-level picker flow + a hash pin that actually matched.** Two defects in the first cut: (1) the fixture hash was derived from the Windows working-tree copy of the submodule, where git checked the cube out with **CRLF** (35,941 CRs) — the real blob is LF and 38 KB smaller. The CI `sha256sum -c` guard caught it before the emulator even booted (the guard working as designed); the pin now comes from `git cat-file blob` (`3e2da957…`, 968,405 bytes) with a comment in both the workflow and the test constants warning never to re-derive it from a working-tree copy. (2) The engine-level test bypassed the actual user flow — tap LUT chip → system file manager → pick file. Added `lutChipOpensSystemPickerAndAppliesDownloadedCube` at UI level: the grade bar's OpenDocument contract is stubbed with **espresso-intents** (`Intents.intending(hasAction(ACTION_OPEN_DOCUMENT))` answering the real downloaded cube's MediaStore `content://` uri) because DocumentsUI is an out-of-process system activity a Compose test cannot drive; `Intents.intended` proves the app launched the picker, then the real callback chain (uri → cache copy → native apply) runs, the chip relabels, and the chip's "clear" item restores the as-shot frame byte-identically. New androidTest dep: `androidx.test.espresso:espresso-intents:3.6.1` (toml `androidxTestEspressoIntents`).
