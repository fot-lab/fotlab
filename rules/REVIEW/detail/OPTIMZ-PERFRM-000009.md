# 缺少断面度量 — `OPTIMZ-PERFRM-000001` 至 `000008` 的量级判断全部未经实测

- ID: OPTIMZ-PERFRM-000009
- Status: Observation
- Priority: P1
- Created: 2026-09-21
- Owner: —
- Related: `rules/REVIEW/detail/OPTIMZ-PERFRM-000001.md`（总览）、`OPTIMZ-PERFRM-000002.md`（量级排序是估算）、`OPTIMZ-PERFRM-000003.md`（降分辨率收益是估算）、`OPTIMZ-PERFRM-000004.md`（PNG 成本是估算）、`OPTIMZ-PERFRM-000007.md`（并行化收益未知，可能带宽受限）

## Background & Goal

`OPTIMZ-PERFRM-000001` 到 `000008` 里所有的"贵/便宜""数百 ms/数 s"都是读代码得出的**数量级估算**，没有一次真机打点。把它们单独记录成一个阻塞项，是为了避免后续任何性能优化在错误的假设上开工——尤其是 `OPTIMZ-PERFRM-000002` 已经指出，这次调研的起因（"FFI 慢"）本身就是一次误判。
...
| # | 待验证假设 | 出现在 |
|---|---|---|
| 1 | `image` crate `PngEncoder::new` 的默认压缩档位不是 `Fast` | `OPTIMZ-PERFRM-000002` / `000004` |
| 2 | PNG deflate + inflate 合计超过 develop 本身 | `OPTIMZ-PERFRM-000002` |
| 3 | Coil 3 在未指定 `size` 时按原始分辨率解码（而非自身有降级策略） | `OPTIMZ-PERFRM-000003` |
| 4 | 50 MP 位图在当前目标设备上确实超过 `GL_MAX_TEXTURE_SIZE` | `OPTIMZ-PERFRM-000003` |
| 5 | 曝光 / calibrate / gamma 三段循环是 CPU 受限还是内存带宽受限 —— 决定 rayon 化的实际收益 | `OPTIMZ-PERFRM-000007` |
| 6 | `Decoder::preview_image` / `thumbnail_image` 对各厂商格式的覆盖率（trait 默认实现返回 `None`） | `OPTIMZ-PERFRM-000006` |
| 7 | 单次渲染峰值是否真的突破 1 GB、是否真的触发 low-memory-kill | `OPTIMZ-PERFRM-000001` |

### 3. 为什么这一项要排在 T0 之前

`OPTIMZ-PERFRM-000007` 的并行化、`OPTIMZ-PERFRM-000004` 的 JNI 直填 Bitmap、`OPTIMZ-PERFRM-000008` 的 GPU 路线，**都是成本不低改造**。若第 2 节第 5 条的答案是"这些循环其实内存带宽受限"，那么并行化的收益会远低于预期；若第 1、2 条的答案是"PNG 其实没那么贵"，那么为搬走 PNG 而新增一层 JNI（属架构变更）就不值得。**先花一次 CI/真机代价把数据拿到，能省掉后面几次误判。**
