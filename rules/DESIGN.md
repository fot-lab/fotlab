...
> **Redirect**: this file is the entry point only. The master item table lives in [`DESIGN/index.md`](DESIGN/index.md); full specifications live in [`DESIGN/detail/`](DESIGN/detail/).
...
3. **Index stays thin** — `index.md` holds the item table only. No statistics, no changelog; git already tracks history.
...
6. **Version numbers are not here** — versioning follows [`VERSION.md`](VERSION.md). A requirement never carries a version number; use status and change history instead.

## Agent Usage Guide

**Read when**

- A feature or requirement task arrives — check `DESIGN/index.md` first for an existing item before proposing anything new.
...
2. Take the next sequence number from `DESIGN/index.md` (6 digits, zero-padded, monotonically increasing across all categories).
3. Create `DESIGN/detail/FOTLAB-{CATEGORY}-{NNNNNN}.md` from the template below.
4. Append exactly one row to the table in `DESIGN/index.md`, linking the new detail file.
...
| 3 | `NNNNNN` | 6-digit zero-padded sequence; one global counter shared by all categories |

Example: `FOTLAB-RENDER-000003`
...
| `NATIVE` | Native Integration | harness layering, JNI/FFI, dnglab integration |
...
- Write detail files in English to stay consistent with `VERSION.md`; the UI-facing copy inside a requirement may be quoted in any language.
