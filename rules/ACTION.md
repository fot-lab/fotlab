...
| `test_image_develop` | `RawlerNativeSmokeTest` develop-entry boundary (3); five RAW brands through the demosaic menu + the every-algorithm/exposure redevelop proof + the manual-Kelvin white-balance no-black regression (7) — 10 total, the heavy software-demosaic shard | RAW corpus |
...
1. **RAW corpus (`fot-lab/rawdb`)** — five camera RAW files (~250 MB: Canon CR2, Sony ARW ×2, Nikon NEF, Panasonic RW2), keyed `rawdb-samples-v1` and never re-keyed, because the samples are immutable test fixtures. Owned by `smoke_emulator.yaml`, cached on the runner and `adb push`ed into the app's external files dir. Without the cache every smoke run would re-download 250 MB for files that never change.
2. **Emulator (AVD + the SDK packages it boots)** — one entry covering `~/.android/avd`, `~/.android/adb*`, `$ANDROID_HOME/system-images`, `$ANDROID_HOME/emulator` and `$ANDROID_HOME/build-tools`, owned by `smoke_emulator.yaml` and keyed by API level + system-image target + arch. It changes only when the AVD definition does, so it is the slowest-changing layer of all. It is written **after** the AVD exists and **before** our APK is installed, so the snapshot stays app-free and reusable (`actions/cache/restore` + `actions/cache/save`, not `actions/cache`).
...
- Instrumented smoke tests live in the AGP-default instrumented source set
  ...
  - `RawlerNativeSmokeTest` — the native bridge survives the two call shapes that
    ...
    `FOTLAB-CRASH-000001`.
  - `RawRoutingTest` — per camera format (Canon CR2, Sony ARW ×2, Nikon NEF,
    Panasonic RW2), the whole RAW chain is traced end to end: sniff dictionary →
    `route` → `StudioEngine` → decoded frame. Its decisive assertion is the
    **size** of the decoded PNG: Coil can at best pull a RAW container's embedded
    preview (a few hundred to ~2k px wide), so a frame narrower than 3000 px means
    the pixels came from a preview and the rawler path did not run. The corpus is
    the public `fot-lab/rawdb` `samples` branch, cached as `rawdb-samples-v1` and
    pushed by `adb` into the emulated SD card (`/sdcard/Pictures/rawdb`) and picked up the way a real photo is: indexed into MediaStore, imported by `LibraryCore.importUris` (the virtual `uri_storage` mapping, no file is ever copied), thumbnail tapped in the real `LibraryScreen` grid, then the viewer's "Open in Studio" button, whose `StudioEngine.setCurrentNode` is what the assertion reads.
    which needs no storage permission. See [Emulator Smoke Test](#emulator-smoke-test).
...
2. Wait for CI.
3. If CI failed, download `build-gradle.log` (and `build_log_gradle.log` for the gradle portion) into the gitignored `log/` directory (see `.gitignore`) and read the compile errors — never commit the logs. A `success` run needs no log download. Kotlin compiler errors are prefixed `e: ` (e.g. `e: file:///...path.kt:42:7 ...`); grep for `^e: ` to extract them quickly.
4. Fix, push again.
