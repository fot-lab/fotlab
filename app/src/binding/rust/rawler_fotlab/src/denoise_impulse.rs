//! Impulse (hot/dead-pixel) denoise sub-stage — pure-functional impulse removal
//! on the scaled mosaic, applied *before* demosaic (the exposure slot).
//!
//! This is the **impulse** half of the composed mosaic denoise. The orchestrator
//! lives in `denoise.rs`; the collaborative (Gaussian / shot) half is
//! `denoise_bm3d_cfa.rs`. Both run on the same 0..1 scaled mosaic the exposure
//! slot hands us, before any demosaic or gain.
//!
...
//!
//! Two enhancements over the textbook RT median test:
//!
...
//! ## Contract
//!
//! `denoise_impulse(pixels, width, height, strength, cfa) -> pixels`. `strength =
//! None` (or `0`) is identity; multi-channel (`cpp > 1`) input is also left
//! untouched by the length check. The orchestrator passes the `CFAConfig` so the
//! stage is colour-aware; `None` (non-CFA) falls back to a single-colour impulse
//! filter. `strength` is a sensitivity multiplier on the detection threshold
