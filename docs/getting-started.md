...
| Rust toolchain | 构建首方绑定 crate `app/src/binding/rust`；其上游 `rawler` 源码来自 `external/dnglab`（`cargo-ndk` 交叉编译到 Android ABI） |
| CMake | 原生构建编排 |
...
| 路径 | 内容 |
| --- | --- |
| `app/src/binding/rust` | 首方 Rust 绑定 crate（唯一产出物：`librawler_fotlab.so`），以 path 依赖引用 `external/dnglab/rawler` |
| `app/src/binding/kotlin/io/github/fotlab/fotlab_rawler` | Kotlin 侧胶水（`RawlerFotlabBridge`）；UniFFI 生成物落在构建目录，不进入源码 |
